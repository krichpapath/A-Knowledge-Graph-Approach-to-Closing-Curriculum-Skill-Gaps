# File: scripts/list_courses_embed.py
"""
List courses ranked for a given job using simple text embeddings (BGE-m3).

Usage:
    python scripts/list_courses_embed.py              # uses first job in DB
    python scripts/list_courses_embed.py 15-1252.00   # specify job_code

This is the "Simple Text Embedding Matcher" baseline:
- job_text  = title + description + tasks
- course_text = title + description
- score(job, course) = cosine(embedding(job_text), embedding(course_text))
"""
...
def compute_embedding_scores(job_text: str, courses: List[Dict]) -> Dict[str, float]:
    """
    Compute similarity scores between job_text and each course text using BGE-m3.
    Returns: {course_id: similarity_score}
    """
    # Build texts list in consistent order
    course_ids = []
    texts = []
    for c in courses:
        cid = c["course_id"]
    ...
    # Embed job and all courses
    job_vec = embed_texts([job_text])[0]      # (d,)
    course_vecs = embed_texts(texts)          # (N, d)
    ...
    sims = course_norms.dot(job_norm)  # (N,)
...
def main():
    ...
    # 4) Compute embedding-based scores
    scores = compute_embedding_scores(job_text, courses)

    # 5) Sort and print top N
    sorted_courses = sorted(courses, key=lambda c: scores.get(c["course_id"], 0.0), reverse=True)

    print(f"\n=== Embedding-based ranking for Job {job_code} ===")
